function [binary threshold globalcosteffmax costmax ef]=threshold_global_cost_efficiency_dir(w,th)
...
no=(d1*(d1-1)); %total no of connections
...
globalcosteffmax=globalcosteff(b);
   ...
   %ef=efficiency_bin(binary);
    %ef=efficiency_wei(w.*binary);
 
[gl_node ef distance binary1]=global_efficiency_wd(1./binary);
