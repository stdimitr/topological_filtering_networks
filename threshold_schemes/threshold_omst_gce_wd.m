...
function [nCIJtree CIJtree mdeg globalcosteffmax costmax E ]=threshold_omst_gce_wd(CIJ,flag)
...
[gl_node E_ini distance binary]=global_efficiency_wd(1./CIJ);
cost_ini=sum(CIJ(:));
...
for  k=1:no_msts
    ...
    [id,od,deg] = degrees_dir(graph);
    degree(k)=mean(deg);
     
    cost(k)=sum(graph(:))/cost_ini;
    [gl_node E distance binary]=global_efficiency_wd(1./graph);
...
costmax=cost(ind);
[gl_node E distance]=global_efficiency_wd(1./CIJtree);
